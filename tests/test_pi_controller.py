...
class TestQrefQuietGate:
    """Tests for the qref-quiet WLS admission gate (clamped_reason='qref_active').

    Empirically validated 2026-05-19: restricting WLS admission to ticks
    where |qref_bias| < threshold removes Y-side endogeneity bias from
    β_solar estimates (|err| 0.81 → 0.15 at 0.05°C threshold on 30-day
    spring bench). See project_qref_y_side_endogeneity memory.

    Lit-grounded sanity-floor remedy: Forssell-Ljung 1999 + Gevers et al.
    2008. Closed-loop endogeneity is best handled by restricting to
    qref-quiet observations where open-loop conditions are recovered.
    """

    def _setup_active_tick(self, threshold: float | None = None,
                            supervisor_enabled: bool = True,
                            supervisor_kind: str = "qref"):
        """Build a controller in a state that will exercise the active-tick
        observation path (HP running, setpoint above room, data complete).
        """
        overrides = {}
        if threshold is not None:
            overrides["pi_supervisor_qref_quiet_gate_c"] = threshold
        if not supervisor_enabled:
            overrides["pi_supervisor_enabled"] = False
        if supervisor_kind != "qref":
            overrides["pi_supervisor_kind"] = supervisor_kind
        config = make_pi_config(overrides)
        entity = FakePIEntity(config)
        pi = entity._pi
        pi._desired_temp = 21.0
        pi._hp_setpoint = 24  # delta = -4, HP definitely on
        entity._attr_hvac_mode = HVACMode.HEAT
        entity._attr_current_temperature = 20.0
        pi._pi_integral = 0.0
        pi._sensor_filter_tau = 0
    ...
    @pytest.mark.asyncio
    async def test_bias_below_threshold_admits(self):
        """|qref_bias| < threshold → observation NOT clamped by gate."""
        entity, pi = self._setup_active_tick(threshold=0.05)
        pi._qref_biaser._bias = 0.02  # below 0.05 threshold
        ...
        assert len(obs) >= 1
        last = obs[-1]
        # gate should NOT fire — clamped_reason should be empty (active obs)
        assert last.clamped_reason != "qref_active", (
            f"Gate fired when bias 0.02 < threshold 0.05; "
            f"clamped_reason={last.clamped_reason!r}"
        )

    @pytest.mark.asyncio
    async def test_bias_above_threshold_clamps_with_qref_active(self):
        """|qref_bias| >= threshold → clamped with reason 'qref_active'."""
        entity, pi = self._setup_active_tick(threshold=0.05)
        pi._qref_biaser._bias = 0.20  # well above 0.05 threshold
        ...
        assert last.clamped_reason == "qref_active", (
            f"Expected clamped_reason='qref_active', got {last.clamped_reason!r}"
        )

    @pytest.mark.asyncio
    async def test_threshold_zero_disables_gate(self):
        """Threshold = 0 disables the gate entirely (back-compat / opt-out)."""
        entity, pi = self._setup_active_tick(threshold=0.0)
        pi._qref_biaser._bias = 0.5  # would normally trigger
        ...
        last = obs[-1]
        assert last.clamped_reason != "qref_active", (
            f"Gate fired with threshold=0 (should be disabled); "
            f"clamped_reason={last.clamped_reason!r}"
        )

    @pytest.mark.asyncio
    async def test_supervisor_disabled_no_gate(self):
        """When supervisor is disabled, gate doesn't fire even with high bias."""
        entity, pi = self._setup_active_tick(
            threshold=0.05, supervisor_enabled=False,
        )
        pi._qref_biaser._bias = 0.5  # high but supervisor disabled
        ...
        last = obs[-1]
        assert last.clamped_reason != "qref_active", (
            f"Gate fired with supervisor disabled; "
            f"clamped_reason={last.clamped_reason!r}"
        )

    @pytest.mark.asyncio
    async def test_governor_kind_no_gate(self):
        """When supervisor_kind='governor' (not qref), gate doesn't fire.

        The gate is specific to qref's bias mechanism; governor uses a
        different (NUDGE-based) bias path that shouldn't be Y-side gated
        by this threshold.
        """
        entity, pi = self._setup_active_tick(
            threshold=0.05, supervisor_kind="governor",
        )
        pi._qref_biaser._bias = 0.5
        ...
        last = obs[-1]
        assert last.clamped_reason != "qref_active", (
            f"Gate fired with kind='governor'; "
            f"clamped_reason={last.clamped_reason!r}"
        )

    @pytest.mark.asyncio
    async def test_existing_clamp_preserved(self):
        """When obs is already clamped (e.g. saturated), gate doesn't overwrite."""
        # Setup state where raw_setpoint will exceed _max_temp_c → saturated_high
        entity, pi = self._setup_active_tick(threshold=0.05)
        pi._max_temp_c = 22.0  # tight max so raw_setpoint will exceed
        pi._qref_biaser._bias = 0.5  # would also trigger qref gate
        # Force large positive integral so raw_setpoint = desired + I + FF >> max
        pi._pi_integral = 100.0
        ...
        assert len(obs) >= 1
        last = obs[-1]
        # Existing reason (saturated_high) should win; qref_active does not overwrite.
        assert last.clamped is True
        assert last.clamped_reason != "qref_active", (
            f"Gate overwrote existing clamped_reason; "
            f"got {last.clamped_reason!r}"
        )

    def test_default_threshold_is_005(self):
        """Production default for the gate threshold is 0.05°C."""
        config = make_pi_config()
        entity = FakePIEntity(config)
        assert entity._pi._supervisor_qref_quiet_gate_c == 0.05
