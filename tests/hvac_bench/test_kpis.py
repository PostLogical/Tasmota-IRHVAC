...
class TestControlKpisComfort:
    def test_inside_deadband_zero_discomfort(self):
    ...
    def test_cold_violation_accumulates_kh(self):
    ...
    def test_warm_violation_accumulates_kh(self):
    ...
    def test_deadband_edge_inclusive(self):
...
class TestControlKpisEnergy:
    def test_total_kwh_tracks_cumulative(self):
    ...
    def test_no_cumulative_kwh_returns_zero(self):
...
class TestControlKpisSettling:
    def test_always_within_deadband_settles_at_zero(self):
    ...
    def test_late_exit_yields_settling_time(self):
...
class TestControlKpisSetpointChanges:
    def test_constant_sp_zero_changes(self):
        history = _fixed_history(n_ticks=20, hp_setpoint=22.0)
        b = compute_control_kpis(history, tick_minutes=15.0)
        assert b.setpoint_changes == 0

    def test_alternating_counts_per_change(self):
        history = _fixed_history(n_ticks=20, hp_setpoint=22.0)
        for i in range(20):
...
class TestAttachLearningKpis:
    def test_bias_is_estimated_minus_truth(self):
    ...
    def test_only_shared_features_appear(self):
        b = attach_learning_kpis(
            _baseline_bundle(),
            beta_estimated={"outdoor_delta": -0.20, "solar": -1.5},
    ...
    def test_passes_diagnostic_flags_through(self):
        b = attach_learning_kpis(
            _baseline_bundle(),
...
class TestCrlbCoverage:
    def test_all_realisations_cover_truth_yields_one(self):
    ...
    def test_no_realisation_covers_yields_zero(self):
    ...
    def test_unidentifiable_counts_as_out(self):
        ests = [{"a": 1.0}, {"a": 1.0}]
...
class TestAggregateMcBundles:
    def test_empty_returns_empty(self):
        assert aggregate_mc_bundles([]) == {}

    def test_summary_stats_per_kpi(self):
        bundles = []
    ...
    def test_settling_time_only_includes_settled(self):
        from dataclasses import replace
