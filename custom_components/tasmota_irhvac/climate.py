...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(
            CONF_PRESET_MODES_LIST, default=DEFAULT_PRESET_MODES_LIST
        ): vol.All(
            cv.ensure_list,
            [vol.In(AVAILABLE_PRESET_MODES_LIST)],
        ),
...
class TasmotaIrhvac(ClimateEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._preset_modes_list = config[CONF_PRESET_MODES_LIST]
                    ...
                    if payload["Vendor"] == self._vendor:
                        ...
                        if "Temp" in payload:
                            if payload["Temp"] > 0:
                                if (
                                    self.power_mode == STATE_OFF
                                    and self._ignore_off_temp
                                ):
                                    self._target_temp = self._target_temp
                                else:
                                    self._target_temp = self._celsius_to_fahrenheit(
                                        payload["Temp"]
                                    )
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        if (
            ...
            and preset_mode != PRESET_SET_V
            and preset_mode != PRESET_SET_H
            and self._economy
        ):
    ...
    async def send_ir(self):
        ...
        payload_data = {
            ...
            "Temp": self._fahrenheit_to_celsius(self._target_temp),
            "FanSpeed": fan_speed,
            "SwingV": self._swingv,
            "SwingH": self._swingh,
            "Quiet": self._quiet,
